...
import com.yeep.basis.swing.widget.frame.TrayFrameHandler;
...
public class MainFrame extends TrayFrame implements TrayFrameHandler
{
	// Services
	...
	private HotKeyHandler pageDownHandler;
	private HotKeyHandler trayHandler;
			...
			public void keyReleased(KeyEvent e)
			{
				...
				case KeyEvent.VK_ALT:
					if (menuBar != null)
					{
						...
						{
							MainFrame.this.setSize(MainFrame.this.getWidth(),
									MainFrame.this.getHeight() + 30);
						}
	...
	{
		registerTrayHandler(KeyHandler.KEY_TRAY.getKeyCode());

		registerPageDownHandler(KeyHandler.KEY_PAGE_DOWN.getKeyCode());
	}

	public void registerPageDownHandler(Integer key)
	{
		if (hotKeyManager == null)
			return;

		if (this.pageDownHandler == null)
		{
			this.pageDownHandler = new HotKeyHandler()
			{
		...
		hotKeyManager.registerHotKey(KeyHandler.KEY_PAGE_DOWN.getKeyCode(),
				pageDownHandler);
	}

	public void registerTrayHandler(Integer key)
	{
		if (hotKeyManager == null)
			return;

		if (this.trayHandler == null)
		{
			this.trayHandler = new HotKeyHandler()
			{
		...
		hotKeyManager.registerHotKey(KeyHandler.KEY_TRAY.getKeyCode(),
				this.trayHandler);
	}
	...
	public void toTray()
	{
		hotKeyManager.unregiesterHotKey(KeyHandler.KEY_PAGE_DOWN.getKeyCode());
	}
	...
	private void initTray()
	{
		ImageIcon icon = new ImageIcon(getClass().getClassLoader().getResource(
				"com/yeep/floatreader/resources/tray.gif"));
		...
		TrayIcon trayIcon = new TrayIcon(icon.getImage(), "Float Reader",
				popopMenu);
	...
	private void initUISettings(Settings uiSettings)
	{
		...
		// Remove all decoration of frame
		...
		if (this.menuBar != null)
			this.menuBar.setVisible(uiSettings.getWindowShowMenu());
	}
	...
	protected void openFile()
	{
		if (this.fileChooser != null
				&& this.fileChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION)
		{
	...
	protected void reloadByRecentFile(RecentFile recentFile) throws IOException
	{
		...
		if (this.pageController == null)
		{
			try
			{
				this.pageController = new PageController(
						recentFile.getFilename(), this.charset,
						this.content.getWidth(), this.content.getHeight(),
						this.content.getFontMetrics(this.content.getFont()),
						recentFile.getIndex());
			}
		...
		{
			this.pageController.reload(recentFile.getFilename(),
					recentFile.getIndex());
		}
	...
	protected void applyCharset(String newCharset)
	{
		...
		catch (IOException e)
		{
			...
			displayErrors(new RuntimeException(
					"Error ocurrs while applying charset " + newCharset));
		}
	}
	...
	protected void changeReaderFont(Font font)
	{
		...
		try
		{
			this.pageController.reload(this.content.getWidth(),
	...
	private PageController loadFile(String filename)
	{
		try
		{
			this.pageController = new PageController(filename, this.charset,
					this.content.getWidth(), this.content.getHeight(),
					this.content.getFontMetrics(this.content.getFont()));
		}
		catch (FileNotFoundException e)
	...
	{
		if (this.pageController == null || this.content == null)
			return;
	...
	private void updateRecentFiles()
	{
		...
		try
		{
			floatReaderService.updateRecentFiles(new RecentFile(
					this.pageController.getFileName(), this.pageController
							.getCurrentCharIndex()));
	...
	private void adjustOpacity(Window window, int rotation)
	{
		...
		System.out.println("1 rotation = " + rotation + ", offset = " + offset
				+ ", opacity = " + opacity);

		if (opacity - offset <= 0.0f)
			opacity = 0.0f;
		else if (opacity - offset >= 1f)
			opacity = 1f;
		else
			opacity -= offset;
	...
	class MouseDragListener extends MouseAdapter
	{
		...
		public void mouseMoved(MouseEvent e)
		{
			...
			if (widthDiff < 10 && widthDiff > -10 && heightDiff < 10
					&& heightDiff > -10)
			{
				isResize = true;
		...
		public void mouseReleased(MouseEvent e)
		{
			if (isResize && MainFrame.this.pageController != null)
			{
				try
				{
					MainFrame.this.pageController.reload(MainFrame.this.content
							.getWidth(), MainFrame.this.content.getHeight(),
							MainFrame.this.content
									.getFontMetrics(MainFrame.this.content
											.getFont()));
		...
		private void resizeComponent(MouseEvent e)
		{
			...
			if ((originWidth <= uiSettings.getWindowWidth() && widthOffset < 0)
					|| (originHeight <= uiSettings.getWindowHeight() && heightOffset < 0))
				return;

			MainFrame.this.setSize(originWidth + widthOffset, originHeight
					+ heightOffset);
		...
		private void moveComponent(MouseEvent e)
		{
			...
			MainFrame.this.setLocation(locationPoint.x + e.getX() - origin.x,
					locationPoint.y + e.getY() - origin.y);
		}
	}
}
